...
2. **截图定位**：游戏中按 `Print Screen`，截图文件名携带玩家坐标与朝向：
   ...
   想让它**定时自动刷新**就用"设置 → 自动截图"：应用隔一段时间替你按一次**配置好的截图键**
   （默认 `PrintScreen`；键盘注入走系统 API，只在局内且游戏窗口在最前时发，见下文"自动截图"一节）。
...
│  ├─ auto-shot.js           定时自动截图（模拟截图键）的决策 + PowerShell 按键助手
│  ├─ annotations.js         手动标注存储（清洗/读写，世界坐标）
│  ├─ room-client.js         房间联机客户端（连服务端、队友位置/轨迹/标注、新局清残留）
│  ├─ quests-data.js         任务快照加载与统计
│  ├─ json-file.js           JSON 读取（兼容 BOM）
...
│  ├─ verify-annotations.js  手动标注验收（21 项，含常驻工具条/椭圆对角拖拽/Shift 正圆）
│  ├─ verify-mini-anno.js    雷达显示标注验收（8 项：三档配置 + 图例联动）
│  ├─ verify-autoshot.js     定时自动截图验收（15 项，干跑模式，不发真实按键）
│  ├─ verify-raid-reset.js   新一局清场验收（11 项，假日志+假截图走完整管线）
│  ├─ verify-room.js         房间联机界面验收（62 项，含三组队友图例、队友开新局清残留、状态提示行不说假话）
...
│  ├─ make-server-exe.js     把服务端打成 Windows 单文件 exe（Node SEA，自带启动自检）
│  ├─ verify-server-exe.js   服务端 exe 端到端验收（起 exe + 两个真客户端走一遍联机）
...
├─ dist-server/              服务端单文件 exe 的产物目录（npm run dist:server，已 gitignore）
├─ samples/                  真实样本（日志 + 截图）
...
- **想让定位定时自动刷新**：设置 → 自动截图（定时刷新定位）→ 勾上开关（默认关），
  间隔与**截图键**都可配置（游戏里改过截图键就填同一个）。开启后只在"局内 + 游戏窗口在最前面"时替你有节奏地按一次，
  连按 3 次都没拿到新坐标会自动暂停并说明原因
- 顶栏：地图下拉、楼**下拉框**（默认"自动"按高度切层）、定位自动居中、**固定地图方向**（默认亮着；关掉 = 随角色朝向旋转）、小地图雷达、标记点、任务、尺子测距、图钉化、导入截图、设置；
  **第二行是常驻的标注工具**（画笔/路径/直线/箭头/椭圆/矩形/橡皮 + 颜色/粗细/撤销/清空/取消）
...
标注工具**常驻顶栏第二行**（不用先点按钮弹出面板）：点任意工具就进入标注模式，画的东西**按地图保存**，
换图/换局/重启都还在。没有"保存/完成"按钮——落笔就自动落盘（600ms 防抖）。
...
| ／ 直线 · ➤ 箭头 | 按住拖：从起点拖到终点 |
| ◯ 椭圆 | **从一个角拖到对角**（像 QQ 截图那样，画出来的是拖拽矩形的内接椭圆）；**按住 Shift 拖 = 正圆** |
| ▭ 矩形 | 从一个角拖到对角 |
| ⌫ 橡皮 | 点一下要删掉的那一笔（线附近 10px 内、椭圆/矩形内部也算） |

- 6 种颜色（红/黄/绿/蓝/紫/白）+ 细/中/粗三档；`撤销` 退掉上一笔，`清空` 只清当前地图
- 快捷键：标注模式下 **Ctrl+Z 撤销**、**回车结束路径**；退出标注点顶栏的 **「取消」**（等价于 **Esc**，
  或**再点一次当前工具**——工具条本身一直留在顶栏；没进标注时「取消」是灰的）
- 退出标注后地图恢复拖动与"定位自动居中"（进入标注时只是临时关掉跟随）
- 标注存的是**世界坐标**（和截图定位同一套），所以缩放、随朝向旋转、换楼层都不会跑位
...
- **旧版画的圆不再保留**：`circle` 这个类型已经被 `ellipse` 取代（椭圆更实用），老数据不做迁移，
  读入时会被丢掉——这是刻意的（见 CHANGELOG/提交说明）
...
- 房间联机时队友的内容分成**三个独立分组**（见"房间联机 → 地图上怎么看"）：
  `队友位置` / `队友轨迹` / `队友绘图`，每组按人一行，可以只关某人的某一样
...
| 雷达上显示标注 | 我的 + 队友的 | `不显示 / 只显示我的 / 我的 + 队友的`：雷达上也画标注（仍受右侧图例开关控制） |
...
**标注也能画在雷达上**（默认就画"我的 + 队友的"，可在"设置 → 圆形小地图雷达 → 雷达上显示标注"里
切成"只显示我的"或"不显示"）：主窗口画完一笔，雷达立刻跟着出现；右侧图例里关掉
`我的标注` 或某个人的 `队友绘图`，两个窗口会**一起**隐藏（图例开关是共用的）。
雷达窗口很小、标注太密时可以只留自己的或干脆关掉。
...
```text
你的客户端 ─┐
队友的客户端 ┼── WebSocket（JSON，一条连接）──> 自建服务端（仓库 server/）
队友的客户端 ┘                                   ├─ docker compose / docker run（推荐）
                                                 └─ 或 Windows 单文件 exe（双击就开，见下）
```
...
```bash
# 1) 起服务端（任一方式）
cd server && docker compose up -d            # 推荐（Linux/NAS/软路由）
# 或者：docker run -d -p 8787:8787 --restart unless-stopped ghcr.io/neppure/tarkov-offline-map-server:2.1.0
# 或者（Windows 不想装 docker）：下载 Release 里的 tarkov-offline-map-server-<版本>-win-x64.exe，双击启动
#      窗口里会直接打印该填什么（服务器地址 = 你的内网 IP，端口 = 8787）
...
npm run room                       # = node server/server.js，默认 8787 端口（也支持 --port/--persist 等开关）
...
#   右侧图例出现「队友位置 / 队友轨迹 / 队友绘图」三行（都在"假队友"名下），
#   关掉"队友绘图"他那个圈就没了、关掉"队友位置"他的标记就没了（轨迹不受影响）
```
...
| 我画的标注 | 画笔/路径/直线/箭头/椭圆/矩形，实时同步；进房时会把当前这张图上你画过的补发一次 |
...
- 右侧图例里队友的内容分成**三个分组**：**`队友位置` / `队友轨迹` / `队友绘图`**，
  每组**一个人一行**（`peer:pos:<id>` / `peer:trail:<id>` / `peer:anno:<id>`），
  可以只关掉某人的某一样：关掉"队友轨迹"他的点还在，关掉"队友位置"轨迹与标注也还在
  （地图上画了什么，图例里一定能单独关掉 —— 这是本项目的硬约束）
- **同一张图开新局时，队友的残留位置会被清掉**：新一局开始那一刻，本机先清掉"进局之前收到"的队友点，
  同时通过服务端的 `newraid` → `peer-reset` 让队友那边也不再显示我的旧点
  （位置是"上一局最后一次按截图键"的快照，不清就会在新局的图上停一个假点）。
  旧版服务端会忽略 `newraid`（本地清除仍生效，远端不生效）——想拿完整效果把 `server/` 一起更新
- 圆形小地图雷达同样会画队友；**队友跑出雷达显示半径时，会按他所在的方位贴到圆边上**
  （图标缩小 + 虚线边框 + 一个朝外的小箭头，一眼看出"人在这个方向的外面"，鼠标悬停能看到距离多少米）
...
npm run dist:server                    # 服务端 Windows 单文件 exe（Node SEA）-> dist-server\...exe（自带启动自检）
node tools/verify-server-exe.js        # 服务端 exe 端到端验收（13 项：--version/--help/真起实例/healthz/
                                       # 两个真客户端进房/位置轨迹/椭圆标注/newraid 清残留/离开/关进程无残留）
...
```bash
npm run dist         # 客户端：electron-builder portable -> dist\塔科夫地图-<version>.exe
npm run dist:server  # 服务端：Node SEA 单文件    -> dist-server\tarkov-offline-map-server-<version>-win-x64.exe
```

- 产物为 **免安装单文件 exe**（v1.2.0 起约 145MB，含 73MB 赛季文件参考截图），双击即用，自带图标与版本信息
- 服务端那条 `npm run dist:server` 走的是 Node 官方的 SEA（把 `server/server.js` + `ws` 打进 blob 再注入 `node.exe`），
  产物约 82MB，**双击即启动**，命令行开关见 [server/README.md](server/README.md)（`--port/--persist/--data-dir/--help`）；
  它是未签名的（postject 注入会丢掉 node.exe 的签名），首次运行 SmartScreen 会提示"未知发布者"
...
`.github/workflows/build.yml`（客户端）：
...
`.github/workflows/server.yml`（服务端，两条交付路径）：

| job | 行为 |
|---|---|
| `test` | 服务端单测 + 集成测试（含命令行参数、`newraid` → `peer-reset`） |
| `win-exe` | Windows 单文件 exe（Node SEA）→ 构建 + **自带启动自检** + `tools/verify-server-exe.js` 端到端验收 → Actions artifact；推 tag 时把 exe 挂到同一个 Release（`tarkov-offline-map-server-<版本>-win-x64.exe` + `SHA256SUMS-server.txt`） |
| `image` | 构建并推 `ghcr.io/neppure/tarkov-offline-map-server`（`main` → `:latest`/`:sha-xxx`；tag → `:X.Y.Z`） |
...
```bash
npm run test:all         # 客户端 161 项 + 服务端 45 项
npm test                 # 客户端单测（解析器/投影/映射 + 赛季数据 + 地图几何/地名文字/拖动平移 + 日志监听 + 任务数据 +
                         ...
                         # 握手超时看门狗 + 成员显示 + 三组队友图例 + 状态提示行 + 雷达贴边钳位 + 雷达标注三档 +
                         # 定时自动截图（键名映射/间隔夹取/跳过与暂停/助手命令协议）+ 主窗口布局硬约束
                         ...
                         # + 人工补录点位 overlay（合并生效/幂等/上游补齐后自动跳过/文件缺失不影响加载）
                         # + 定位精度回归（解析不丢小数 / 投影往返 <1e-9m / 1cm 不量化 / 各图比例尺自洽 /
                         #   朝向不被整数化——防止以后有人手一抖加取整）），161 个用例
...
node tools/verify-annotations.js           # 手动标注验收（21 项：常驻工具条/六种工具落笔/椭圆对角拖拽/Shift 正圆/橡皮/撤销/图例开关/换图不串味/重载后仍在/
                                           # 再点工具或 Esc 退出；标注文件先落盘备份，被强杀也能下次自动还原）
node tools/verify-mini-anno.js             # 雷达显示标注验收（8 项：默认"我的+队友的" / 三档切换 / 与图例开关联动）
node tools/verify-autoshot.js              # 定时自动截图验收（15 项；**必须用 TAKOV_AUTOSHOT_DRY=1 或 --autoshot-dry 启动客户端**，
                                           # 干跑模式只记状态不发按键：间隔节流/连续无效自动暂停/键名归一化/间隔夹取）
...
- `renderer/common/map-view.js` 的"手动标注"段：绘制/橡皮/撤销/命中判定 + 椭圆几何；`renderer/map.js` 的"手动标注"段负责常驻工具条与持久化

定时自动截图（定时刷新定位）相关：
- `src/auto-shot.js` 决策 + 按键助手（纯 Node，`test/auto-shot.test.js` 覆盖；Windows 上用常驻 PowerShell 发一次系统按键）
- `main.js` 的"定时自动截图"段：把配置/局内状态喂给 runner，并按 5 秒节流把状态广播给界面
- 只在**局内（日志里出现过进图行）+ 游戏窗口在最前面**时按；连续 3 次没拿到新坐标自动暂停
- `tools/verify-autoshot.js` 用干跑模式验收（`TAKOV_AUTOSHOT_DRY=1`，只记状态不发按键）

## 免责声明

本项目仅读取玩家自己的游戏日志与截图文件，不注入进程、不读写游戏内存。
**"定时自动截图"**（默认关）是唯一会向外发送输入的功能：它通过 Windows 的系统 API
（`keybd_event`）发一次你在设置里指定的按键，让游戏自己写出带坐标的截图 —— 不注入游戏进程、
不读写游戏内存，并且只在"局内 + 游戏窗口在最前面"时才发（连按 3 次无效会自动暂停）。
即便如此它仍然是"合成输入"，请自行评估使用风险，**建议只在离线/单人局使用**。
