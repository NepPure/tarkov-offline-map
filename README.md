...
表层显示全部标记、自动切换图层、声音提示（只用于战局：匹配/匹配到了/进图倒计时，可调"最后几秒"）、自动删除截图文件、小地图雷达参数等）均持久化到本地设置文件。
设置里的**游戏日志目录 / 截图目录**除了手输路径，还能点「**选择文件夹…**」用系统文件夹选择框挑
（只填进输入框，点「保存」才生效），旁边的「**打开文件夹**」直接在资源管理器里打开当前路径。
...
│  ├─ raid-alerts.js         战局提示音状态机（匹配等待服务器 / 匹配到了 / 进图倒计时最后几秒）
│  ├─ room-client.js         房间联机客户端（连服务端、队友位置/轨迹/标注/勾选任务、新局清残留）
...
│  ├─ verify-mini-quest.js   雷达与主地图一致验收（10 项：勾选的任务点也画在雷达上 + 雷达上的字不会整体消失）
│  ├─ verify-raid-alerts.js  战局提示音验收（11 项：自起客户端 + 假日志，验三个时刻响 / 定位与换图不响 / 倒计时落点）
│  ├─ verify-quest-share.js  共享勾选 + 进图要带验收（24 项：自起服务端 + 假队友，验合并显示 / 角标 / 筛选 / 图例 / 黄色高亮）
...
- 顶栏：地图下拉、楼**下拉框**（默认"自动"按高度切层）、定位自动居中、**固定地图方向**（默认亮着；关掉 = 随角色朝向旋转）、小地图雷达、标记点、任务、尺子测距、导入截图、设置；
...
| 面板顶部 chips | `本图`（只列当前地图有内容的）/ `有地点`（只列能画出来的）/ `已勾选`（**我**勾的）/ `队友勾选`（房间共享的、**队友**勾的）/ `击杀区`（默认关）。两个勾选筛选是**与**关系：只开一个 = 只看那一方，两个都开 = 只看我和队友**都**勾了的（找"一起做"的任务） |
| 两个下拉 | 按**商人**、按**等级上限**过滤 |

- **一眼看出这个任务是谁勾的**：任务行上有角标——自己勾的是青色 `我`，队友勾的是**他在地图上的颜色 + 昵称**
  （长昵称截断，悬停看全名）；**展开明细**第一行就是「谁勾选：你 + 阿甘」（房间共享了勾选才有队友）；
  面板底部统计行也会写 `· 队友勾选 N`，一眼知道队友那边共享了多少任务。
  「队友勾选」筛选在**没联机 / 服务端不支持共享**时会给出解释性提示（说明需要哪三件事），而不是干瘪的空列表。
...
- 展开任务明细时，**最上面是一块黄色高亮的「进图要带」**（钥匙 / 要带进图放置·使用的任务物品），
  免得开局走到门口才发现钥匙没带：
  - **钥匙**：来自目标上的 `requiredKeys`；一组里有多把时按"**或**"显示
    （`宿舍306钥匙 或 宿舍308钥匙、机械钥匙` = 前两把任意一把，另加后面那把）
  - **物品**：放置类（`plantItem/plantQuestItem`）与使用类（`useItem`）的任务物品，带数量
  - **刻意不列**：`findItem/findQuestItem`（是进图去**捡**）与 `giveItem/giveQuestItem`（在任务界面**交**）——
    列进来只会让清单变长、还误导人
  - 折叠状态的任务行上也会有一个黄色角标（`🔑带2` / `📦带1`），一眼看出这任务要带东西
  - 同一个任务点开后弹的**详情卡**里也有同一块（用的同一份纯函数 `questBringList`）
...
**勾选的任务点也画在雷达上**（任务区域多边形 + 物品刷新点虚线圈）：和主窗口读同一份
`data/quests-dump.json`、同一份勾选状态、同一个透明度与"显示击杀区"开关，两边永远一致；
换图或勾选变化时才重算图层（状态推送很频繁，不做每帧重算）。
转移点文字同样会补上中文目的地（用和主窗口同一份"地图 id → 中文名"字典）。
"设置 → 定位与显示 → 表层显示全部标记"也同时作用于两个窗口。
...
为避免小圆盘里"糊成一团"，雷达会**按视口裁剪**并按"重要度 + 离圆心距离"降噪：

- 标记数超过 90 个时，丢掉**最不重要的、最远的**（散落物资、各类物资箱 → 远处的地名…），
  只保留撤离点/Boss/转移点/钥匙锁/开关/危险/固定武器/赛季文件/BTR 站点等关键点，
  以及**离你最近的那几个地名**
- 再超过 260 个（还没定位、视野被拉到很宽时）同样按重要度 + 距离硬截断
- 关键标记的**中文名**最多同时标 6 个，超过时取"最重要的 + 最近的"若干个
  （旧版是"超过 6 个就一个名字都不写"，于是把半径调大或走进撤离点密集区时，
  雷达上的字会整体消失、来回跳——已修）
...
**提示音只用于战局节奏**，三个时刻各一种音色（`设置 → 提示与行为` 里可总开关 + 调"最后几秒"）：

| 时刻 | 音色 | 日志依据（`|application|` 后的消息开头） | 实测间隔 |
|---|---|---|---|
| ① 开始匹配（在等服务器） | 一声低音 | `Matching with group id: N` | — |
| ② 匹配到了 | 三声上行 | `MatchingCompleted:` | ① 之后 **197~209 秒** |
| ③ 进图倒计时的最后几秒 | 短促滴一声（3/2/1 各一次） | 倒计时从 `GameSpawned:` 开始，`GameStarting:` 是结束 | `GameSpawned → GameStarting` **10.3 秒**（4 局全一样） |

- **截图定位、队友位置、换图都不会响** —— 这些是常态高频事件，响起来只会烦人（旧版"进图/定位各响一声"已删掉）
- 倒计时的几声落在两条日志之间，所以主进程按"`GameSpawned` + 实测 10.3 秒"排定时器；
  `GameStarting` 比预计来得早（组队里别人先加载完）时，没响完的会**立刻补一声**，不会静悄悄进图
- 默认"最后 3 秒"开始响（1~10 秒可调）；应用启动/重连时回补的历史日志（回扫 2MB）
  里那些几分钟前的匹配行**不会**补响一串
- 每次响铃都会写进 `app.log`（`[alert] 响铃 match-queue` / `... countdown（还剩 2 秒）`），
  想核对时机直接看日志时间戳

实测时间线（`1.1.5.1.47510`，PvE，4 个会话 6 局；完整原行在 `test/raid-alerts.test.js` 里）：

```
Matching with group id: N          开始匹配（等待服务器）
  +1.7s    scene preset            （已经在预载地图）
  +16~28s  LocationLoaded
  +197~209s MatchingCompleted      匹配到了
  +0.5s    GamePrepared/GameCreated
  +17~18s  GamePooled
  +7s      GameRunned → GameSpawn → PlayerSpawnEvent
  +10.3s   GameSpawned             倒计时开始（最后 3 秒响）
...
# 或者：docker run -d -p 8787:8787 --restart unless-stopped ghcr.io/neppure/tarkov-offline-map-server:2.2.0
...
| 我勾选的任务 | 整份任务 id 列表（默认开，可在设置里关）。队友图上会画出你勾的任务；**两人都勾了同一个任务时只画一次**（合并显示），鼠标悬停能看到是谁勾的 |
| 昵称 | 地图上用**昵称第一个字** + 朝向箭头画出来；重名会补短 id（`小明#3f2`） |

**不共享**：游戏日志、截图文件、图例开关、设置、你的任何本地文件。
服务端只转发上面那几样；房间号在传输前已哈希，**不在 URL 里、日志里也只打前 6 位**。

> 共享勾选任务是**加能力**（服务端 `welcome` 里声明 `caps: ['quests']`），**没有升协议大版本**：
> 新客户端连老服务端 = 这项功能静默关闭（位置/标注照旧），老客户端连新服务端 = 忽略不认识的字段。
> 想用这个功能要把 `server/` 一起更新（`docker compose up -d --build` / 重新发 exe）。
...
- **队友勾选的任务**画在同一个任务图层里，图例的「任务标记」组里会多出**每人一行**的
  **「XX勾选的任务」**（`quest:peer:<id>`，颜色就是他在地图上的颜色）。规则：
  - 你和他都勾了同一个任务 → **只画一次**（合并），悬停显示「任务名（你 + 阿甘 勾选）」
  - 只被他勾的任务 → 关掉他那行就一起隐藏；只被你勾的 → 永远画（不看队友开关）
  - 图例里的计数是"他在这张图上有几个任务有位置"，不是点数
  - 左侧任务面板也能看出来：行上有 `我` / 队友昵称角标，展开明细第一行写「谁勾选」，
    顶部还有 `队友勾选` 筛选（详见「任务侧边栏怎么用」）
- 圆形小地图雷达同样会画队友勾选的任务（合并规则一样）
...
npm test                 # 客户端单测（解析器/投影/映射 + 赛季数据 + 地图几何/地名文字/拖动平移 + 日志监听 + 任务数据 +
                         ...
                         #   朝向不被整数化——防止以后有人手一抖加取整）
                         # + 雷达与主地图一致（勾选的任务点也画在雷达上 / 同一份勾选与透明度 /
                         #   地图中文名字典 / 表层标记开关）与"雷达上的字不会整体消失"回归
                         #   （上限裁剪与标名按"重要度 + 离圆心距离"挑，绝不整类丢、绝不清空）
                         # + 设置页目录按钮（日志/截图目录的「选择文件夹…」「打开文件夹」接线与
                         #   只选目录的 IPC）
                         # + 战局提示音（解析真实日志行 / 状态机三个时刻各响一次 / 倒计时落在最后几秒 /
                         #   历史回放不响 / 提前进图补一声；并断言"定位与换图不再有提示音"）
                         # + 「进图要带」清单（钥匙按"或"分组、只收放置/使用类物品、去重、脏数据不抛；
                         #   真实任务数据里 ≥40 个任务能挑出钥匙）+ 黄色高亮接线
...
npm run visual-test      # 真实输入事件自检：滚轮缩放/拖拽/测距/标记图钉开关/赛季文件/小地图拖动与焦点，截屏到 test-artifacts/
...
node tools/verify-mini-quest.js            # 雷达与主地图一致验收（10 项：勾选的任务点在雷达上也画出来、两边 questItems 一致、
                                           # 关掉「任务区域」两个窗口一起隐藏、30 个撤离点同时在场也标出最近的 6 个 —— CDP 只读 DOM，不动系统鼠标）
node tools/verify-raid-alerts.js           # 战局提示音验收（11 项，**自己起隔离客户端 + 往假日志里追加真实格式的日志行**：
                                           # 开始匹配/匹配到了/倒计时最后几秒各响一次、截图定位与换图不响、
                                           # 倒计时落点（GameSpawned 后 8~11 秒）、提前进图补一声、app.log 有 [alert] 行）
node tools/verify-quest-share.js           # 队友共享勾选任务 +「进图要带」验收（24 项，**自己起隔离客户端 + 真服务端 + 假队友**：
                                           # 勾选共享出去/收进来、同一任务两人都勾只画一次（合并）、我·队友角标、
                                           # 「谁勾选」明细、队友勾选筛选与交集语义、图例「XX勾选的任务」开关只隐藏"只他勾"的、
                                           # 进图要带黄色高亮与钥匙）
...
node tools/verify-quests.js                # 任务侧边栏验收（41 项：搜索/分组/勾选/图层/图例覆盖/详情卡 + "位置在别的图"提示与一键切图，CDP 合成点击，不动系统鼠标）
...
**本轮新增功能（战局提示音 / 共享勾选任务 / 进图要带 / 任务面板队友勾选）的验证状态**：
单测/集成测试之外，三项都写了**自起隔离实例**的端到端验收并且已经跑绿：
`node tools/verify-raid-alerts.js`（11/11）、`node tools/verify-quest-share.js`（24/24）、
`node tools/verify-mini-quest.js`（10/10，需先 `--remote-debugging-port` 起客户端）。
回归也跑过：`verify-quests.js` 41/41、`verify-room.js` 62/62、`verify-raster.js` 28/28、
`verify-mini-anno.js` 8/8、`verify-raid-reset.js` 11/11、`verify-about.js` 15/15、
`verify-autoshot.js` 15/15（干跑）、`npm run visual-test`（renderer errors: NONE）。
