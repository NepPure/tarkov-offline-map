 ...
 *   1) 标题去掉"离线" -> 塔科夫地图（之前误写成"塔可夫"，全库统一改回正确写法）
...
/** 用户可见的文本文件里一个"塔可夫"都不能再留（游戏叫「逃离塔科夫」） */
const TEXT_FILES = [
  'renderer/map.html', 'renderer/map.js', 'renderer/map.css', 'renderer/minimap.js',
  'main.js', 'preload.js', 'package.json', 'README.md',
  'server/README.md', 'server/package.json', 'server/docker-compose.yml', 'server/server.js',
  '.github/workflows/build.yml',
];

test('品牌名：一律写「塔科夫」，不再出现误写的「塔可夫」', () => {
  for (const f of TEXT_FILES) {
    assert.ok(!read(f).includes('塔可夫'), `${f} 里还有误写的「塔可夫」`);
  }
  const pkg = JSON.parse(read('package.json'));
  assert.strictEqual(pkg.productName, '塔科夫地图');
  assert.ok(pkg.build.portable.artifactName.startsWith('塔科夫地图-'), `exe 名应为「塔科夫地图-<版本>.exe」，实际 ${pkg.build.portable.artifactName}`);
});

test('品牌名：标题里不再有"离线"，左上角不再有"纯本地"徽标', () => {
  const html = read('renderer/map.html');
  assert.ok(html.includes('<title>塔科夫地图</title>'), '窗口标题应为「塔科夫地图」');
  assert.ok(html.includes('<h1>塔科夫地图</h1>'), '左上角标题应为「塔科夫地图」');
  assert.ok(!html.includes('塔科夫离线地图'), 'map.html 里不该再有旧名字');
  assert.ok(!html.includes('纯本地'), '左上角「纯本地」徽标应已移除');

  const main = read('main.js');
  assert.ok(main.includes("const APP_TITLE = '塔科夫地图';"), '窗口标题常量应改名为「塔科夫地图」');
  assert.ok(!main.includes('塔科夫离线地图'), 'main.js 里不该再有旧名字');

  const pkg = JSON.parse(read('package.json'));
  assert.strictEqual(pkg.productName, '塔科夫地图');
  assert.strictEqual(pkg.build.productName, '塔科夫地图');
  ...
  assert.ok(!yml.includes('塔科夫离线地图'), 'Release 标题不该再有旧名字');
});
